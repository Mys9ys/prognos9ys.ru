...
class EstateProductionOrderService
{
    ...
    public function claimOrder(int $userId, int $orderId, int $qty = 0): array
    {
        ...
        $chunk = min(
            LaborExchangeConfig::MAX_ESTATE_CLAIM_QTY,
            $remaining,
            $maxByMaterials
        );
        if ($qty > 0) {
    ...
    public function submitFromInventory(int $userId, int $orderId, int $qty = 0): array
    {
        ...
        $chunk = $this->resolveSubmitChunk($remaining, $userHave, (float)($order['UF_COIN_ESCROW'] ?? 0), $payPerCycle);
        if ($qty > 0) {
            $chunk = min($chunk, $qty);
        }
        if ($chunk <= 0) {
    ...
    {
        return [
            'max_claim_qty' => LaborExchangeConfig::MAX_ESTATE_CLAIM_QTY,
