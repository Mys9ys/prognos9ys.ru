...
const SRC = path.join('C:', 'Users', 'RobotComp', '.cursor', 'projects', 'd-OSPanel-home-prognos9ys', 'assets');
const OUT = path.join(__dirname, '..', 'src', 'assets', 'icons', 'soft');
const PREVIEW_OUT = path.join(__dirname, '..', 'src', 'assets', 'icons', 'previews');
const SIZE = 128;

const MAP = {
  'prognobak-soft.png': { out: 'prognobak.png', letter: true },
  'rublius-soft-v2.png': { out: 'rublius.png' },
  'chest-wc2026-soft.png': { out: 'chest-wc2026.png' },
  'bank-soft.png': { out: 'bank.png' },
  'trophy-soft.png': { out: 'trophy.png' },
  'ball-soft.png': { out: 'football.png', football: true },
};

const PREVIEW_MAP = {
  'ball-flat.png': { out: 'ball-flat.png' },
  'ball-pixel.png': { out: 'ball-pixel.png' },
  'ball-soft.png': { out: 'ball-soft.png', football: true },
};
...
function isGreyCheckerboard(r, g, b) {
  ...
  return spread <= 18 && min >= 165 && max <= 238;
}
...
function removeBackground(pixels, width, height, options = {}) {
  ...
  floodRemove(pixels, width, height, (r, g, b) => isDarkNeutral(r, g, b));
  floodRemove(pixels, width, height, (r, g, b, x, y) => isRemovableWhite(pixels, width, height, x, y));
...
async function addPrognobakLetter(buffer, width, height) {
  const svg = Buffer.from(`<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
    <text x="50%" y="47%" text-anchor="middle" dominant-baseline="middle"
      font-family="Arial, Helvetica, sans-serif" font-weight="700" font-size="${Math.round(width * 0.34)}"
      fill="#1f6b28" stroke="#0f3f16" stroke-width="1.5" paint-order="stroke">п</text>
  </svg>`);

  return sharp(buffer).composite([{ input: svg, top: 0, left: 0 }]).png().toBuffer();
}

async function processFile(srcPath, outPath, options = {}) {
  ...
  if (options.football) {
    convertFootballToBw(pixels);
  }

  removeBackground(pixels, info.width, info.height, options);
  const cropped = squareCrop(pixels, info.width, info.height);

  let pipeline = sharp(cropped.pixels, {
    raw: { width: cropped.width, height: cropped.height, channels: 4 },
  }).resize(SIZE, SIZE);

  if (options.letter) {
    const buf = await pipeline.png().toBuffer();
    const withLetter = await addPrognobakLetter(buf, SIZE, SIZE);
    fs.writeFileSync(outPath, withLetter);
  } else {
    await pipeline.png({ compressionLevel: 9 }).toFile(outPath);
  }

  const stat = fs.statSync(outPath);
  console.log('OK', path.basename(outPath), stat.size, 'bytes');
}
...
(async () => {
  ...
  for (const [srcName, cfg] of Object.entries(MAP)) {
    ...
    await processFile(srcPath, path.join(OUT, cfg.out), cfg);
  }

  for (const [srcName, cfg] of Object.entries(PREVIEW_MAP)) {
    const srcPath = path.join(SRC, srcName);
    if (!fs.existsSync(srcPath)) {
      console.log('SKIP preview', srcName);
      continue;
    }
    await processFile(srcPath, path.join(PREVIEW_OUT, cfg.out), cfg);
  }
})();
