...
class PlayoffSlotHelper
{
    ...
    /**
     * @param array<string, mixed> $row нормализованная строка или legacy PROPERTY_*
     */
    public static function isPlayoffMatch(array $row, ?string $stageDetailCode = null): bool
    {
        $stageDetail = trim((string)($row['stage_detail'] ?? ''));
        if ($stageDetail === '' && $stageDetailCode) {
            $stageDetail = self::readPropertyValue($row, $stageDetailCode);
        }
        ...
        $stage = mb_strtolower(trim((string)($row['stage'] ?? $row['PROPERTY_STAGE_VALUE'] ?? '')));
        ...
        $bracketCode = trim((string)($row['bracket_code'] ?? $row['PROPERTY_BRACKET_CODE_VALUE'] ?? ''));

        return $bracketCode !== '';
    }

  /** @deprecated используйте isPlayoffMatch */
    public static function isPlayoffMatchRow(array $res, ?string $stageDetailCode = null): bool
    {
        return self::isPlayoffMatch($res, $stageDetailCode);
    }
